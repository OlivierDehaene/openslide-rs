...
fn test_basic_metadata() {
    ...
    assert_eq!(slide.level_downsample(2).unwrap().floor(), 4.);
    assert_eq!(slide.level_downsample(3).unwrap().floor(), 8.);
...
fn test_properties() {
    ...
    assert_eq!(
        slide.property("openslide.vendor").unwrap().unwrap(),
        "generic-tiff"
    );
}
