...
use openslide_rs::{Address, DeepZoom, OpenSlide, Size, Region};
...
fn read_region_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("read_tile", |b| {
        b.iter(|| slide.read_region(black_box(Region { address: Address { x: 0, y: 0 }, level: 0, size: Size { w: 512, h: 512 } })))
    });
}
